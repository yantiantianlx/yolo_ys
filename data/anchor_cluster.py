...
def compute_centroids(boxes, n_anchors, loss_convergence, grid_size, iterations_num, plus):
    ...
    center_list = list()
    for centroid in centroids:
        center_list.append([round(centroid.w, 2), round(centroid.h, 2)])
...
if __name__ == '__main__':
    ...
    test_root_path = '/simple_ssd/ys2/human_detection_data/test'
    test_json_path = os.path.join(test_root_path, 'json')

    boxes = list()
    for json_file in tqdm(os.listdir(test_json_path)):
        abs_json_path = os.path.join(test_json_path, json_file)
        jsn = json.load(open(abs_json_path))
    ...
    compute_centroids(boxes, n_anchors=8, loss_convergence=0, grid_size=7, iterations_num=1000, plus=0)
