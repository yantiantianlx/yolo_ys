...
def train(arg: ARG, train_loader, net, loss_func, optimizer, epoch, vis: VIS=None):
    bbox_format = list()
    bbox_format.append(Bbox_Format(arg.model.net.im_size, arg.model.net.feature_size[0], arg.model.anchor, arg.model.net.num_class, arg.model.mask_iou_threshold, arg.model.filter_iou_threshold))
    ...
    for batch_idx, batch_data_dict in enumerate(train_loader):
        ...
        if batch_idx % arg.train.log_iteration_interval == 0:
            ...
            image_show = torch_im_to_cv2_im(batch_image[0])
            cx, cy, w, h = (batch_label[0][0].data.cpu().numpy()[1:])
            x1, y1, x2, y2 = cxcywh_to_x1y1x2y2(cx, cy, w, h)
            x1, y1, x2, y2 = list(map(lambda x: int(x), (x1, y1, x2, y2)))
            image_tar_show = cv2.rectangle(image_show.copy(), (x1, y1), (x2, y2), (0, 255, 0))
            vis.image('_0_target', image_tar_show)
...
def main():
    ...
    # =======================================================
    hand_dataset = Hand_Dataset(arg, arg.data.dataset.train.root, arg.data.dataset.train.relative_txt_path)
    ...
    for epoch in range(1, arg.train.epochs + 1):
        ...
        if epoch % arg.train.save_model_epoch_interval == 0:
            torch.save(net.state_dict(), os.path.join(arg.save.model, 'snapshot_epoch{}_model'.format(epoch)))
