...
from tst.tster_body_yolo import Tester
from valid.valider_body_yolo import Valider
...
class Trainer(nn.Module):
    def __init__(self, arg=None, vis=None, id=None):
        ...
        self.dataset = Hand_Dataset(self.arg, self.arg.data.dataset.train.root, im_size=self.arg.model.net.im_size,
        ...
        self.net = YoloV3_Micro(class_num=self.arg.model.net.num_class, anchors_num=self.arg.model.net.num_anchor)
    ...
    def save_hyper_parameter(self, save_path):
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        with open(save_path, 'w') as f:
            json.dump(self.hyper_parameter, f)
    ...
    def step_vis(self, batch_data_dict, net_out, whole_loss, losses_list, predict_list, target_list):
        ...
        for layer_idx, (losses, batch_predict, batch_target) in enumerate(
            ...
            self.vis.line('layer' + str(layer_idx) + '/loss_dx', y=loss_dx.item())
            self.vis.line('layer' + str(layer_idx) + '/loss_dy', y=loss_dy.item())
            self.vis.line('layer' + str(layer_idx) + '/loss_w', y=loss_w.item())
            self.vis.line('layer' + str(layer_idx) + '/loss_h', y=loss_h.item())
            self.vis.line('layer' + str(layer_idx) + '/loss_confidence', y=loss_confidence.item())
            # self.vis.line(str(layer_idx)+'/loss_class', y=sum(loss_class.tolist()))

    def train_steps(self, num_step):
        ...
        for _ in range(num_step//len(self.dataloader)+1):
            for batch_idx, batch_data_dict in enumerate(self.dataloader):
                print('worker'+str(self.id)+' train_step{}-->{} now:{}'.format(begin_step, last_step, self.global_step))
