...
class ARG_DATA_DATASET_TEST(Cls):
    def __init__(self):
        ...
        self.root = '/simple_ssd/ys2/human_detection_data/test'
...
class ARG_DATA_DATASET_TRAIN(Cls):
    def __init__(self):
        ...
        self.max_detection_num = 10
        self.min_h = 0.2
        self.min_w = 0.1
        self.root = '/simple_ssd/ys2/human_detection_data/train'
        self.txt_path = join('/simple_ssd/ys2/human_detection_data/train', 'pair.txt')
...
class ARG_MODEL_NET(Cls):
    def __init__(self):
        super().__init__()
        self.feature_size = [[7, 7], [14, 14]]
        self.gear_cls_tree_path = 'arg.model.net'
        self.im_size = [224, 224]
        self.num_anchor = len([[0.21, 0.71], [0.27, 0.55], [0.39, 0.39], [0.3, 1.0], [0.39, 0.77], [0.55, 0.55], [0.5, 1.0], [0.71, 0.71]])
        self.num_class = 0
...
class ARG_MODEL(Cls):
    def __init__(self):
        super().__init__()
        self.flt_anchor = [[0.21, 0.71], [0.27, 0.55], [0.39, 0.39], [0.3, 1.0], [0.39, 0.77], [0.55, 0.55], [0.5, 1.0], [0.71, 0.71]]
        ...
        self.weight_path = join(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0]))), 'model/body_micro_yolo/init_weight')
...
class ARG_OPTIM(Cls):
    def __init__(self):
        ...
        self.lr = 0.001
        self.lr_decay_mode = 'cosine_decay_restarts'
...
class ARG_SAVE(Cls):
    def __init__(self):
        ...
        self.hyper = join(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0]))), 'train/res', 'body_micro_yolo_aug1'+'_'+'ys2'+'_'+config_running_time)
        self.model = join(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0]))), 'train/res', 'body_micro_yolo_aug1'+'_'+'ys2'+'_'+config_running_time)
        self.root = None
        self.tensorboard = join(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0]))), 'train/res/tensorboard', 'body_micro_yolo_aug1'+'_'+'ys2'+'_'+config_running_time)
...
class ARG(Cls):
    def __init__(self):
        super().__init__()
        self.gear_abs_config_file_name = '/home/ys/Desktop/tinyyolo/gear_config/yolo_config/body_micro_yolo_aug1.yaml'
